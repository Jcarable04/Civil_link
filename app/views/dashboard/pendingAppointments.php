...
<div class="min-h-screen p-4 md:p-8 lg:p-12">
    <div class="max-w-7xl mx-auto">
        ...
        <!-- Header -->
        <header class="mb-8">
            <h1 class="text-4xl font-extrabold text-gray-800 tracking-tight border-b-4 border-indigo-500 pb-2 inline-block">
                Pending Appointments
            </h1>
            <p class="text-gray-500 mt-2">Review and manage all appointments awaiting approval.</p>
        </header>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <?php if (!empty($pendingAppointments)): ?>
                            <?php foreach ($pendingAppointments as $idx => $appt): ?>
                                <tr class="striped-row hover:bg-indigo-50 transition duration-150 ease-in-out">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-indigo-600 truncate max-w-[150px] md:max-w-none" title="<?= htmlspecialchars($appt['email']) ?>"><?= htmlspecialchars($appt['email']) ?></td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                        <!-- Approve Button -->
                                        <a href="/admin/approve/<?= $appt['id'] ?>" class="btn inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="8" class="px-6 py-12 whitespace-nowrap text-center text-lg text-gray-400">
                                    ...
                                    <p class="mt-4">No pending appointments found.</p>
                                </td>
                            </tr>
                        <?php endif; ?>
                        </tbody>
